...
class TestAttachmentsAPI:
    ...
    def test_get_attachments(self, attachments_api: AttachmentsAPI) -> None:
        ...
        with patch.object(attachments_api, '_api_request') as mock_request:
            ...
            result = attachments_api.get_attachments(entity_type="case", entity_id=1)
            
            mock_request.assert_called_once_with('GET', 'get_attachments/case/1')
            assert len(result) == 2

    @pytest.mark.parametrize("entity_type", ["case", "run", "plan", "project"])
    def test_get_attachments_different_entity_types(self, attachments_api: AttachmentsAPI, entity_type: str) -> None:
        """Test get_attachments with different entity types."""
        with patch.object(attachments_api, '_api_request') as mock_request:
            mock_request.return_value = [{"id": 1}]
            
            result = attachments_api.get_attachments(entity_type=entity_type, entity_id=1)
            
            mock_request.assert_called_once_with('GET', f'get_attachments/{entity_type}/1')

    def test_add_attachment_minimal(self, attachments_api: AttachmentsAPI) -> None:
        ...
        with patch.object(attachments_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('POST', 'add_attachment/case/1', data=expected_data)
            assert result == {"id": 1, "name": "file.txt"}

    def test_add_attachment_with_description(self, attachments_api: AttachmentsAPI) -> None:
    ...
    def test_add_attachment_without_description(self, attachments_api: AttachmentsAPI) -> None:
        ...
        with patch.object(attachments_api, '_api_request') as mock_request:
            ...
            expected_data = {"file": "/path/to/file.txt"}
            mock_request.assert_called_once_with('POST', 'add_attachment/run/1', data=expected_data)
    ...
    def test_get_attachment_content(self, attachments_api: AttachmentsAPI) -> None:
        ...
        with patch.object(attachments_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('GET', 'get_attachment_content/1')
            assert result == b"file content"

    def test_api_request_failure(self, attachments_api: AttachmentsAPI) -> None:
    ...
    def test_authentication_error(self, attachments_api: AttachmentsAPI) -> None:
