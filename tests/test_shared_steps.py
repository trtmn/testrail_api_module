...
class TestSharedStepsAPI:
    ...
    def test_add_shared_step_minimal(self, shared_steps_api: SharedStepsAPI) -> None:
        ...
        with patch.object(shared_steps_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('POST', 'add_shared_step/1', data=expected_data)
            assert result == {"id": 1, "title": "New Shared Step"}

    def test_add_shared_step_with_description(self, shared_steps_api: SharedStepsAPI) -> None:
    ...
    def test_update_shared_step(self, shared_steps_api: SharedStepsAPI) -> None:
        """Test update_shared_step method."""
        with patch.object(shared_steps_api, '_api_request') as mock_request:
            mock_request.return_value = {"id": 1, "title": "Updated Shared Step"}
            ...
            mock_request.assert_called_once_with('POST', 'update_shared_step/1', data=expected_data)

    def test_delete_shared_step(self, shared_steps_api: SharedStepsAPI) -> None:
        ...
        with patch.object(shared_steps_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('POST', 'delete_shared_step/1')
            assert result == {}

    def test_api_request_failure(self, shared_steps_api: SharedStepsAPI) -> None:
    ...
    def test_authentication_error(self, shared_steps_api: SharedStepsAPI) -> None:
