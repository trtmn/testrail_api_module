...
class TestVariablesAPI:
    ...
    def test_get_variable(self, variables_api: VariablesAPI) -> None:
        ...
        with patch.object(variables_api, '_api_request') as mock_request:
            mock_request.return_value = {"id": 1, "name": "var1", "value": "value1"}
    ...
    def test_add_variable_minimal(self, variables_api: VariablesAPI) -> None:
        ...
        with patch.object(variables_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('POST', 'add_variable/1', data=expected_data)
            assert result == {"id": 1, "name": "var1", "value": "value1"}

    def test_add_variable_with_description(self, variables_api: VariablesAPI) -> None:
    ...
    def test_update_variable(self, variables_api: VariablesAPI) -> None:
        ...
        with patch.object(variables_api, '_api_request') as mock_request:
            mock_request.return_value = {"id": 1, "name": "updated_var", "value": "updated_value"}
            ...
            mock_request.assert_called_once_with('POST', 'update_variable/1', data=expected_data)
    ...
    def test_get_variable_groups(self, variables_api: VariablesAPI) -> None:
        ...
        with patch.object(variables_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('GET', 'get_variable_groups/1')
            assert len(result) == 2

    def test_add_variable_group_minimal(self, variables_api: VariablesAPI) -> None:
        """Test add_variable_group with minimal required parameters."""
        with patch.object(variables_api, '_api_request') as mock_request:
            mock_request.return_value = {"id": 1, "name": "New Group"}
            
            result = variables_api.add_variable_group(project_id=1, name="New Group")
            
            expected_data = {"name": "New Group"}
            mock_request.assert_called_once_with('POST', 'add_variable_group/1', data=expected_data)
            assert result == {"id": 1, "name": "New Group"}

    def test_add_variable_group_with_description(self, variables_api: VariablesAPI) -> None:
    ...
    def test_update_variable_group(self, variables_api: VariablesAPI) -> None:
        ...
        with patch.object(variables_api, '_api_request') as mock_request:
            ...
            result = variables_api.update_variable_group(
                group_id=1,
                name="Updated Group"
            )
            
            expected_data = {"name": "Updated Group"}
            mock_request.assert_called_once_with('POST', 'update_variable_group/1', data=expected_data)
    ...
    def test_delete_variable_group(self, variables_api: VariablesAPI) -> None:
        ...
        with patch.object(variables_api, '_api_request') as mock_request:
            ...
            mock_request.assert_called_once_with('POST', 'delete_variable_group/1')
            assert result == {}
