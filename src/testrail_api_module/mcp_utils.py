...
def discover_api_methods(
        api_instance: TestRailAPI) -> Dict[str, List[Callable]]:
    """
...
def create_api_from_env() -> TestRailAPI:
    ...
    if not base_url or not username:
        logger.error(
            "Missing required environment variables: TESTRAIL_BASE_URL and/or TESTRAIL_USERNAME")
        raise ValueError(
    ...
    if not api_key and not password:
        logger.error(
            "Missing authentication: Neither TESTRAIL_API_KEY nor TESTRAIL_PASSWORD is set")
        raise ValueError(
